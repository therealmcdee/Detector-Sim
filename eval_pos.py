...
def main(argv):
    ...
    if os.path.exists("data_text_files/{}.txt".format(file_name)) == True:
        ...
        #if len(argv) > 1 :
            #nrun = int(argv[1])
        for root,dirs,files in os.walk("/media/Data_Storage_3/richard_UCNA+/"):
            for f in files:
                if f.startswith('pos'):
                    file_seg = f.split('_')
                    test_ene = file_seg[2]
                    ...
                    ave_y0 = []
                    index = 0
                    if '{}_{}_{}_'.format(file_seg[0],file_seg[1],file_seg[2]) == file_name:
                        print('{}{}'.format(file_name,test_pos))

                        fdata = ROOT.TFile("/media/Data_Storage_3/richard_UCNA+/{}{}".format(file_name,test_pos),"READ")
        ...
        #hittree.Branch("hits",hits,"hits[128]/F")
            ...
            # get the total number of sipm hits
                            ...
                            #stddev_x.append(calc_stddev(xp))
                            #stddev_y.append(calc_stddev(yp))
            
                            new_file = open("data_text_files/{}{}.txt".format(file_name,test_pos),"a")
                            new_file.write("{}keV {} {} {} {} {} {} ".format(test_ene,x0,y0,xp[index],yp[index],resx[index],resy[index]))
    ...
    #fdata.Write("",TFile.kOverWrite)
                        ...
                        stddev_x.append(calc_stddev(xp))
                        stddev_y.append(calc_stddev(yp))
                        xpos.append(np.sum(ave_x0)/len(ave_x0))
                        ypos.append(np.sum(ave_x0)/len(ave_y0))
        
    plt.plot(xpos,stddev_x,color='b')
    plt.plot(ypos,stddev_y,color='r')
